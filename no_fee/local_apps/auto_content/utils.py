...
def get_tweets():
	...
	list_data = []
	most_recent_tweet = ExtractedArticle.objects.all().aggregate(Max('TweetId'))
	print most_recent_tweet
	# for i in range(1,8,1):
	# 	ld = twitter.get_list_statuses(slug='first-test-content-list',owner_screen_name='captainhurst',include_rts=1,page=i,count=100, since_id=most_recent_tweet)
	# 	list_data.append(ld)
	ld = twitter.get_list_statuses(slug='first-test-content-list',owner_screen_name='captainhurst',include_rts=1,count=800, since_id=most_recent_tweet)
	print ld
	list_data.append(ld)
...
def get_article_stuff(html):
	...
	try:
		imgtag = soup.find("img")['src']
	except:
		imgtag = False

	article_imgs_desc['article_images'] = imgtag 
	...
	article_imgs_desc['article_meta_image'] = metaimg
	...
	for i in range(len(links)):
		if links[i]['url'] == False:
			...
			try:
				...
				# print response
				html = response.read()
				links[i]['article_details'] = get_article_stuff(html)
				html = Document(html).summary()
				text = strip_html_tags(html)
				links[i]['article_details']['article_text'] = text
				art = ExtractedArticle(
					Source = links[i]['url'], 
					HatTipHandle = links[i]['user_handle'], 
					HatTipName = links[i]['ht'], 
					TweetId = links[i]['tweet_id'], 
					TweetText = links[i]['text'],
					TweetImage = links[i]['tweet_image'], 
					RawArticleText = links[i]['article_details']['article_text'], 
					ArticleImage = links[i]['article_details']['article_images'],
					ArticleMetaImage = links[i]['article_details']['article_meta_image'],
					MetaDescription = links[i]['article_details']['description']
					)
				art.save()
			except:
				pass
...
def tweet_object(tweets):
	...
	for i in range(len(tweets)):
		for ld in range(len(tweets[i])):
			excheck = ExtractedArticle.objects.filter(TweetId = tweets[i][ld]['id'])
			if excheck.count() > 0:
				pass
			else:	
				try:
				...
				try:
					handle = tweets[i][ld]['user']['screen_name']
				except:
					handle = False	
...
def create_articles(link_articles):
